...
{//block是传入的起始逻辑块地址
    ...
    /*如果叶子结点没有一个有效的ext4_extent结构，直接return，path[ppos]->p_ext保持NULL*/
	if (eh->eh_entries == 0) {
		/*
		 * this leaf is empty:
...
叶子节点                                    ext4_extent_header + ext4_extent + ext4_extent+ ........
    ...
    */
	...
	while (i) {
        ...
        //通过索引节点ext4_extent_idx结构的ei_leaf_lo和ei_leaf_hi成员计算出的物理块号，这个物理块保存了下层叶子节点或者索引节点4K数据
		path[ppos].p_block = ext4_idx_pblock(path[ppos].p_idx);//
		path[ppos].p_depth = i;//逻辑块地址接近map->m_lblk的索引节点或叶子节点所在ext4 extent B+树中的层数
        ...
        */
        //path[ppos].p_block是保存了下层叶子节点或者索引节点4K数据，bh映射指向这个物理块
        ...
        //eh指向当前索引节点对应的 下层的索引节点或者叶子节点的头结点，注意，是当前ppos索引节点下层的索引节点或者叶子节点
		eh = ext_block_hdr(bh);
        //索引节点层数加1
		ppos++;
        ...
        //上边ppos++了，ppos代表下一层索引节点或者叶子节点了。path[ppos].p_bh指向新的ppos这一层 索引节点或者叶子节点 4K数据的物理块 
        //映射的bh
		path[ppos].p_bh = bh;
        //path[ppos].p_bh指向ppos这一层索引节点或者叶子节点的头结构ext4_extent_header
		path[ppos].p_hdr = eh;
		i--;
  ...
  //的ext4_extent，令path[ppos]->p_ext指向这个ext4_extent。如果叶子结点没有一个ext4_extent结构，则path[ppos]->p_ext保持NULL
	...
	if (path[ppos].p_ext)
        //由ext4_extent结构的ee_start_hi和ee_start_lo成员计算出的物理块号，这个物理块号是ext4_extent的逻辑块地址映射的的起始物理块号
				 ...
				 int logical, ext4_fsblk_t ptr)
{
	...
	if (logical > le32_to_cpu(curp->p_idx->ei_block)) {
		/* insert after */
        //待插入的ext4_extent_idx结构起始逻辑块地址logical大于curp->p_idx的起始逻辑块地址， 就要插入curp->p_idx这个ext4_extent_idx
        //后边，(curp->p_idx + 1)这个ext4_extent_idx后边。插入前，下边memmove先把(curp->p_idx+1)后边的所有ext4_extent_idx结构全向后
        //移动一个ext4_extent_idx结构大小，然后把新的ext4_extent_idx插入到curp->p_idx + 1位置处
		ext_debug("insert new index %d after: %llu\n", logical, ptr);
		ix = curp->p_idx + 1;
	} else {
		/* insert before */
        //待插入的ext4_extent_idx结构起始逻辑块地址logical更小，就插入到curp->p_idx这个ext4_extent_idx前边。插入前，下边memmove
        //先把curp->p_idx后边的所有ext4_extent_idx结构全向后移动一个ext4_extent_idx结构大小，
        //然后把新的ext4_extent_idx插入到curp->p_idx位置处
		ext_debug("insert new index %d before: %llu\n", logical, ptr);
		ix = curp->p_idx;
	}
    //ix是curp->p_idx或者(curp->p_idx+1)。len是ix这个索引节点的ext4_extent_idx结构到索引节点最后一个ext4_extent_idx结构(有效的)之间
    //所有的ext4_extent_idx结构个数。注意，EXT_LAST_INDEX(curp->p_hdr)是索引节点最后一个有效的ext4_extent_idx结构，如果索引节点只有
    //一个ext4_extent_idx结构，那EXT_LAST_INDEX(curp->p_hdr)就指向这第一个ext4_extent_idx结构
	...
	if (len > 0) {
		ext_debug("insert new index %d: "
				...
				logical, len, ix, ix + 1);
        //把ix后边的len个ext4_extent_idx结构向后移动一次ext4_extent_idx结构大小
    ...
    //现在ix指向ext4_extent_idx结构是空闲的，用它保存要插入的逻辑块地址logial和对应的物理块号。相当于把本次要插入ext4 extent b+树的ext4_extent_idx插入到ix指向的ext4_extent_idx位置处
	ix->ei_block = cpu_to_le32(logical);
	ext4_idx_store_pblock(ix, ptr);
    //索引节点有效的ext4_extent_idx增加一个，因为刚才新插入了一个ext4_extent_idx
	le16_add_cpu(&curp->p_hdr->eh_entries, 1);
    /*隐藏点，竟然没有对ei_len赋值，我去，ext4_extent_idx结构没有ei_len成员*/
...
2:因为ext4_extent B+树at那一层索引节点有空闲entry，则针对at~depth(B+树深度)之间的的每一层索引节点和叶子节点
都分配新的索引节点和叶子结点，每个索引节点和叶子结点都占一个block大小(4K)，分别保存N个ext4_extent_idx结构
和N个ext4_extent结构，还有ext4_extent_header。在while (k--)那个循环，这些新分配的索引节点和叶子节点中，B+树倒数第2层的那个索引节点的
第一个ext4_extent_idx的物理块号成员(ei_leaf_lo和ei_leaf_hi)记录的新分配的保存叶子结点4K数据的物理块号
...
at那一层新分配的索引节点(物理块号是newblock，起始逻辑块地址border)，执行ext4_ext_insert_index()插入到ext4_extent B+树at
层原有的索引节点(path + at)->p_idx指向的ext4_extent_idx结构前后的ext4_extent_idx结构位置处。插入过程是:把
...
出了一个空闲的ext4_extent_idx结构大小空间，新分配的索引节点就是插入到这里。
...
针对ext4_extent B+树at~denth新分配索引节点和叶子节点物理块号映射bh内存。这是对原有的ext4 extent B+树进行扩容的重点。
*/
...
的ext4_extent结构还是ex，只是逻辑块范围减少了。而后半段ext4_extent结构即newext就要插入插入到到ext4 extent B+树。到ext4_ext_insert_extent()
函数，如果此时ex所在叶子节点的ext4_extent结构爆满了，即if (le16_to_cpu(eh->eh_entries) < le16_to_cpu(eh->eh_max))不成立，但是
...
当执行到ext4_ext_split()，at一层的ext4_extent B+树有空闲entry，则以从at层到叶子节点那一层，创建新的索引节点和叶子节点，建立这些
新的索引节点和叶子节点彼此的物理块号的联系。我们假设ext4_ext_split()的if (path[depth].p_ext != EXT_MAX_EXTENT(path[depth].p_hdr))
成立，则这样执行:

向新分配的叶子节点复制m个ext4_extent结构时，复制的第一个ext4_extent结构不是path[depth].p_ext，而是
...
不会向新分配的叶子节点复制ext4_extent结构，m是0，因为path[depth].p_ext就是叶子节点最后一个ext4_extent
...
注意，这是ext4_extent B+树叶子节点增加增加的第一个ext4_extent结构，并且第一个ext4_extent结构的起始逻辑块地址与它上边的索引节点
的ext4_extent_idx的起始逻辑块地址都是newext->ee_block，再上层的索引节点的ext4_extent_idx的起始逻辑块地址也是newext->ee_block，
直到第at层

因此，我们看到ext4_ext_split()最核心的作用是:at一层的ext4_extent B+树有空闲entry，则从at层开始创建新的索引节点和叶子节点，
...
有了空闲的ext4_extent entry，把newex插入到老的path[depth].p_ext所在叶子节点后边即可。或者新创建的at~denth的索引节点
...
于是从ext4 extent B+树at那一层索引节点到叶子节点，针对每一层都创建新的索引节点，也创建叶子节点。还会尝试把
索引节点path[at~depth].p_hdr指向的ext4_extent_idx结构的后边的ext4_extent_idx结构和path[depth].p_ext指向的
ext4_extent结构后边的ext4_extent结构，移动到新创建的叶子节点和索引节点。这样可能保证ext4 extent B+树中，
与newext->ee_block有关的叶子节点有空闲entry，能存放newext。*/
			  ...
			  struct ext4_extent *newext, int at)
{
	...
	if (path[depth].p_ext != EXT_MAX_EXTENT(path[depth].p_hdr)) {
        //path[depth].p_ext不是叶子节点最后一个ext4_extent结构，那以它后边的ext4_extent结构path[depth].p_ext[1]的起始逻辑块地址作为分割点border
        /*走到这个分支，下边把向新分配的叶子节点复制m个ext4_extent结构时，复制的第一个ext4_extent结构不是path[depth].p_ext，而是
        它后边的 path[depth].p_ext[1]这个ext4_extent结构。并且，下边新创建的索引节点的第一个ext4_extent_idx结构的起始逻辑器块地址
        都是border，即path[depth].p_ext[1]的逻辑块地址，即path[depth].p_ext[1].ee_block。然后向新传创建的索引节点的第2个
        ext4_extent_idx结构位置处及之后复制m个ext4_extent_idx结构。新传创建的索引节点的第一个ext4_extent_idx的起始逻辑块地址是border，
        单独使用，作为分割点的ext4_extent_idx结构。如此，后续执行ext4_ext_find_extent(newext->ee_block)在老的ext4_extent B+树找到的
        path[depth].p_ext指向的ext4_extent还是老的，但是path[depth].p_ext后边的m个ext4_extent结构移动到了新分配的叶子节点，
        path[depth].p_ext所在叶子节点就有空间了，newext就插入到path[depth].p_ext指向的ext4_extent叶子节点后边。这段代码在
        ext4_ext_insert_extent()的has_space 的if (!nearex)........} else{......}的else分支*/
	   ...
	   /*走到这个分支，下边不会向新分配的叶子节点复制ext4_extent结构，m是0，因为path[depth].p_ext就是叶子节点最后一个ext4_extent
	   结构，下边的m = EXT_MAX_EXTENT(path[depth].p_hdr) - path[depth].p_ext++=0。并且，下边新创建的索引节点的第一个ext4_extent_idx结构
	   的起始逻辑器块地址都是newext->ee_block。这样后续执行ext4_ext_find_extent()在ext4_extent B+树就能找到起始逻辑块地址是
	   newext->ee_block的层层索引节点了，完美匹配。还有一点，向新传创建的索引节点的第2个ext4_extent_idx结构位置处及之后复制
	   m个ext4_extent_idx结构，这个m会不会也是0呢，不一定，因为path[i].p_idx指向的ext4_extent_idx结构不一定是索引节点最后一个
	   ext4_extent_idx结构呀!是的话m才是0。
	   需注意，这个分支没有向新的叶子节点复制ext4_extent结构，空的，
	   ext4_ext_find_extent()执行后，path[ppos].depth指向新的叶子节点的头结点，此时直接令该叶子节点的第一个ext4_extent结构的
	   逻辑块地址是newext->ee_block，相当于把newex直接插入到该叶子节点的第一个ext4_extent结构位置处!
	 ...
	 * upon them.
	 */
	//依照ext4_extent B+树层数分配depth个ext4_fsblk_t的数组，下边保存分配的物理块号
	...
	for (a = 0; a < depth - at; a++) {
        //从ext4文件系统元数据区分配一个物理块，返回它的物理块号，4K大小，保存ext4 extent B+树索引节点或者叶子结点的
        //头结构ext4_extent_header+N个ext4_extent_idx或者N个ext4_extent结构
    ...
    //neh指向新分配的叶子节点首内存的头结构ext4_extent_header，下边对新分配的叶子节点头结构ext4_extent_header进行初始化
    ...
    //i初值是ext4_extent B+树最后一层索引节点的层数，就是叶子节点上边的那层索引节点
	i = depth - 1;
	//循环k次保证把at那一层的ext4_extent B+树索引节点到最后一层索引节点中，每一层索引节点path[i].p_idx指向的ext4_extent_idx结构到最后
	//一个ext4_extent_idx结构之间的ext4_extent_idx结构，都复制到上边新创建的索引节点的物理块中，物理块号是ablocks[--a]即newblock，
	//bh映射这个物理块。neh指向这个索引节点头ext4_extent_header结构，fidx是这个索引节点第一个ext4_extent_idx结构。
	//注意，是从ext4_extent B+树最下层的索引节点向上开始复制，因为i的初值是depth - 1，
	//这是ext4_extent B+树最下边一层索引节点的层数
	while (k--) {
		oldblock = newblock;
        //新取出一个物理块对应的块号newblock，这个物理块保存索引节点ext4_extent_header头结构+N个ext4_extent_idx结构的4K数据
		newblock = ablocks[--a];
        //newblock物理块映射的bh
		bh = sb_getblk(inode->i_sb, newblock);
		if (unlikely(!bh)) {
		...
		if (err)
			goto cleanup;
        //neh指向newblock这个物理块映射的bh的内存首地址，这片内存打头的是索引节点的头ext4_extent_header结构
		neh = ext_block_hdr(bh);
		neh->eh_entries = cpu_to_le16(1);
		neh->eh_magic = EXT4_EXT_MAGIC;
        //计算索引结点能容纳的ext4_extent_idx结构个数
		neh->eh_max = cpu_to_le16(ext4_ext_space_block_idx(inode, 0));
        //索引节点所处B+树层数
		neh->eh_depth = cpu_to_le16(depth - i);
        //fidx指向索引节点的第一个ext4_extent_idx结构
		fidx = EXT_FIRST_INDEX(neh);
        //fidx的起始逻辑块地址是上边的分割点逻辑点地址border
		fidx->ei_block = border;
        /*重点，第一次while循环到这里，oldblock(即newblock)是保存上边新分配的叶子节点ext4_extent_header头结构+N个ext4_extent结构
         的4K数据的物理块号，fidx指向最后一层索引节点(就是叶子节点上边的那层索引节点)
         第一个ext4_extent_idx结构，这里是把保存新分配的保存叶子结点的4K数据的物理块号newblock保存到最后一层索引节点第一个索引节点的
         ext4_extent_idx结构中。后续的循环，都是新分配的上层索引节点的第一个ext4_extent_idx结构记录 保存下层新分配的索引节点4K数据
         (ext4_extent_header头结构+N个ext4_extent_idx结构)的物理块号。
         说白了，ext4_ext_split()函数把原有的ext4_extent B+树at~depth层的索引节点和叶子结点的后半段数据移动到新创建的
         索引节点和叶子结点中。这里是在上层索引节点中记录 保存下层索引节点或者叶子结点4K数据的物理块号*/
        //把下一层叶子节点或者下一层索引节点的物理块号保存到当前索引节点第一个ext4_extent_idx结构的ei_leaf_lo和ei_leaf_hi成员中。
        //后续可以通过这个ext4_extent_idx结构的ei_leaf_lo和ei_leaf_hi成员找到它指向的下一层的索引节点或者叶子节点
       ...
       /*隐藏点，path[i].p_idx++，执行后path[i].p_idx指向下一个ext4_extent_idx结构。这样下边memmove(++fidx, path[i].p_idx...)向
		...
		if (m) {
			...
			if (err)
				goto cleanup;
            //path[i].p_hdr指向的老的ext4 extent B+树那一层索引节点减少了m个ext4_extent_idx结构
        ...
        //i--进入下次循环，就会把上一层ext4_extent B+树索引节点的path[i].p_idx指向的ext4_extent_idx结构到最后一个ext4_extent_idx结构之间
...
B+树原来的at那一层的(path + at)->p_idx指向的ext4_extent_idx结构前后，实质是在这个位置插入一下新的ext4_extent_idx结构，它
的起始逻辑块号成员是border，它的起始物理块号成员是newblock。ext4 extentB+树原来的at那一层有空闲的entry，就是有空闲的位置存放
新的索引节点。*/
				 ...
				 struct ext4_extent *newext)
{
    ...
    //把ext4 extent B+树的根节点的数据(头结构ext4_extent_header+4个ext4_extent_idx或者叶子结点ext4_extent_header+4个ext4_extent结构)
    //复制到bh->b_data。相当于把根节点的数据复制到上边新创建的物理块，腾空根节点
	...
	neh->eh_entries = cpu_to_le16(1);
    //这是把前边新创建的索引节点或者叶子节点的物理块号newblock记录到根节点第一个ext4_extent_idx结构的ei_leaf_lo和ei_leaf_hi成员。
    //这样就建立了根节点与新创建的物理块号是newblock的叶子结点或索引节点的联系。因为通过根节点第一个ext4_extent_idx结构的
    //ei_leaf_lo和ei_leaf_hi成员，就可以找到这个新创建的叶子节点或者索引节点的物理块号newblock
	...
	if (neh->eh_depth == 0) {
        ...
        //以前B+树只有根节点，没有索引节点，根节点都是ext4_extent结构，现在B+树根节点下添加了newblock这个叶子节点。根节点成了索引节点，
        //因此原来第一个ext4_extent结构要换成ext4_extent_idx结构，下边赋值就是把原来的根节点第一个ext4_extent的起始逻辑块地址
        //赋值给现在根节点的第一个ext4_extent_idx的起始逻辑块地址
 ...
 */
/*执行到该函数，说明ext4 extent B+树中与newext->ee_block有关的叶子节点ext4_extent结构爆满了，需要扩容。首先尝试搜索叶子节点上的每一层
索引节点有没有空闲entry的，有的话记录这一层索引节点的深度是at。接着执行ext4_ext_split():从ext4 extent B+树at那一层索引节点到叶子
节点，针对每一层都创建新的索引节点，也创建叶子节点。还会尝试把索引节点path[at~depth-1].p_hdr指向的ext4_extent_idx结构的后边的
ext4_extent_idx结构和path[depth].p_ext指向的ext4_extent结构后边的ext4_extent结构，移动到新创建的叶子节点和索引节点。这样就可能保证
ext4 extent B+树中，与newext->ee_block有关的叶子节点有空闲entry，能存放newext。如果ext4 extent B+树索引节点的ext4_extent_idx结构
也爆满了，则执行ext4_ext_grow_indepth()在ext4 extent B+树root节点下的创建一层新的索引节点(或者叶子节点)。此时ext4 extent B+树第2层的
索引节点(或者叶子节点)是空的，可以存放多个ext4_extent_idx结构，即有空闲entry了。然后大概率goto repeat处，执行ext4_ext_split()分割
创建索引节点和叶子节点。总之，从ext4_ext_create_new_leaf()函数返回前，里边执行的ext4_ext_find_extent()找打的path[depth].p_ext指向
的叶子节点有空闲entry，可以存放newext*/
...
repeat:
	...
	//节点或叶子节点有空闲ext4_extent_idx或ext4_extent可使用，2:i是0，ext4 extent B+树索引节点或叶子节点ext4_extent_idx或ext4_extent个数爆满，没有空闲ext4_extent_idx或ext4_extent可使用
    ...
    //ext4 extent B+树索引节点或者叶子节点有空闲ext4_extent_idx或ext4_extent可使用。此时的i表示ext4 extent B+树哪一层有空闲
    //ext4_extent_idx或ext4_extent可使用。newext是要插入ext4_extent B+树的ext4_extent，插入ext4_extent B+树的第i层的叶子节点或者
    //第i层索引节点下边的叶子节点*/
	if (EXT_HAS_FREE_INDEX(curp)) {
		...
		/*凡是执行到ext4_ext_split()函数，说明ext4 extent B+树中与newext->ee_block有关的叶子节点ext4_extent结构爆满了。
           于是从ext4 extent B+树at那一层索引节点到叶子节点，针对每一层都创建新的索引节点，也创建叶子节点。还会尝试把
           索引节点path[at~depth].p_hdr指向的ext4_extent_idx结构的后边的ext4_extent_idx结构和path[depth].p_ext指向的
           ext4_extent结构后边的ext4_extent结构，移动到新创建的叶子节点和索引节点。这样就能保证ext4 extent B+树中，
           与newext->ee_block有关的叶子节点有空闲entry，就能存放newext这个ext4_extent结构了。*/
		...
		/* refill path */
		ext4_ext_drop_refs(path);
        //ext4_ext_split()对ext4_extent B+树做了重建和分割，这里再次在ext4_extent B+树查找起始逻辑块地址接近newext->ee_block
        //的索引节点和叶子结点
	...
	/*到这个分支，ext4 extent B+树索引节点的ext4_extent_idx和叶子节点的ext4_extent个数全爆满，全都没有空闲条目entry，
	  就是说ext4 extent B+树全爆满了，只能增加执行ext4_ext_grow_indepth()增加ext4 extent B+树叶子节点或者索引节点了。理解这点非常关键*/
        ...
        //下方新增加的索引节点有空闲entry。如果此时ext4 extent B+树叶子节点有空闲entry，则从ext4_ext_create_new_leaf()返回后可直接把
		 ...
		 */
		depth = ext_depth(inode);
        //path[depth].p_hdr指向的叶子结点保存ext4_extent结构达到eh_max，即ext4_extent结构爆满，则goto repeat寻找有空闲ext4_extent_idr的索引节点，
        //然后分割ext4 extent B+树。这if大概率是成立的，因为之所以执行到ext4_ext_create_new_leaf()，就是因为很多叶子结点的ext4_extent
        //结构爆满了，而上边ext4_ext_find_extent()只是在ext4 extent B+树root节点下边增加了一层索引节点(或者叶子节点)而已。
 ...
 */
/**/
/*回到eh这个叶子节点上层的索引节点，找到path[depth-1].p_idx指向的ext4_extent_idx，这个索引节点结构ext4_extent_idx的起始逻辑块地址
最接近传入的逻辑块地址map->m_lblk，接着找到紧挨着这个ext4_extent_idx结构后边的ext4_extent_idx，这个ext4_extent_idx的起始
逻辑块地址就可能大于newext->ee_block，即本次要插入ext4 extent B+树的ext4_extent(即newext)的起始逻辑块地址。这样的话newex就插入
新找到的索引节点ext4_extent_idx的下边的叶子节点，最后返回这个新的ext4_extent_idx的起始逻辑块地址。
如果找不到这个索引节点ext4_extent_idx，返回EXT_MAX_BLOCKS*/
...
static ext4_lblk_t ext4_ext_next_leaf_block(struct ext4_ext_path *path)
{
  ...
  /*执行到这里，说明原来path[depth].p_ext所在叶子节点的所有ext4_extent逻辑块地址范围太小，要插入ext4 extent B+树
  的新ext4_extent即newext起始逻辑块地址太大。则先找到path[depth-1].p_ext所在叶子节点上层的索引节点的ext4_extent_idx结构，
  我们这里称为ext4_extent_idx1.这个ext4_extent_idx在ext4_ext_find_extent->ext4_ext_binsearch_idx()中找到并赋值，
  具体是从ext4 extent B+树从上层到下层，依次在每一层索引节点中查找哪个ext4_extent_idx起始逻辑块地址最接近
  传入的要查找逻辑块地址block，然后path[depth-1].p_idx=ext4_extent_idx1。

  现在要返回ext4_extent_idx1后边的ext4_extent_idx结构，后续newex就是要插入这个ext4_extent_idx下边的叶子节点。

  为什么要这样操作，因为newext，是要插入path[depth-1].p_idx指向的索引节点ext4_extent_idx1下的叶子节点(即path[depth].p_ext所在叶子节点)
  ，但是newext的起始逻辑块地址大于path[depth].p_ext所在叶子节点所有ext4_extent结构的起始逻辑块地址。那肯定要找个起始逻辑块地址
  更大的索引节点ext4_extent_idx，于是就找到path[depth-1].p_idx指向的索引节点ext4_extent_idx后的
  ext4_extent_idx1，newex就是要插入到ext4_extent_idx1下边的索引节点。

  ext4 extent B+树索引节点的是一个个ext4_extent_idx从左到右组成的，从左到右，每个ext4_extent_idx的逻辑块
  起始地址依次增大。叶子节点的ext4_extent结构也是从左到右起始逻辑块地址依次增大。

  这里有个问题，万一新找到的ext4_extent_idx起始逻辑块地址还是太小咋办?

  并且，上边的解释是理想情况，在depth-1那一层索引节点就找到的path[depth-1].p_idx指向的索引节点ext4_extent_idx后的
  ext4_extent_idx1，万一下边if (path[depth].p_idx != EXT_LAST_INDEX(path[depth].p_hdr))不成立，
  就只能再depth---去倒数第2层索引节点找了,依然没找到再depth---去倒数第3层索引节点找

  以上解释后来我自己都觉得太啰嗦了
  */
	if (depth == 0)//B+树只有根节点直接返回EXT_MAX_BLOCKS
        /* zero-tree has no leaf blocks at all */
		return EXT_MAX_BLOCKS;

	/* go to index block */
	depth--;//depth--就到ext4_extent B+树的最下层的索引节点了

	while (depth >= 0) {
        //path[depth].p_idx指向起始逻辑块地址最接近传入的起始逻辑块地址map->m_lblk的ext4_extent_idx，
        //EXT_LAST_INDEX(path[depth].p_hdr)是ext4 extent B+树索引节点最后一个ext4_extent_idx。二者不能相等，
        //因为这里是return path[depth].p_idx后边的ext4_extent_idx即path[depth].p_idx[1].ei_block。
		if (path[depth].p_idx != EXT_LAST_INDEX(path[depth].p_hdr))
            //path[depth].p_idx[1],即path[depth].p_idx指向的ext4_extent_idx后边的那个ext4_extent_idx结构
            /*隐藏点，如果path[depth].p_idx[1]指向的索引节点ext4_extent_idx结构并没有使用过，则path[depth].p_idx[1].ei_block
              是全0xFFFFFFF，即EXT_MAX_BLOCKS，相当于没有找到有效的ext4_extent_idx*/
...
/*首先尝试把newext合并到ex(即path[depth].p_ext)、或者(ex+1)、或者(ex-1)指向的ext4_extent结构，合并条件很苛刻，合并成功则直接返回。
接着看ext4 extent B+树中与newext->ee_block(这个要插入B+树的ext4_extent结构的起始逻辑块地址)有关的叶子节点是否ext4_extent结构爆满，
即是否有空闲entry。没有空闲entry的话就执行ext4_ext_create_new_leaf()创建新的索引节点和叶子节点，这样就可以保证
ext4_ext_create_new_leaf()->ext4_ext_find_extent()执行后path[depth].p_ext指向的ext4_extent结构所在的叶子节点有空闲entry，可以存放
newext。接着是该函数has_space分支，只是简单的把newex插入path[depth].p_ext前后的ext4_extent位置处。最后还会执行ext4_ext_try_to_merge()
尝试把ex后的ext4_extent结构的逻辑块和物理块地址合并到ex，还会尝试把叶子结点的ext4_extent结构移动到root节点，节省空间
*/
int ext4_ext_insert_extent(handle_t *handle, struct inode *inode,
				struct ext4_ext_path *path,
				struct ext4_extent *newext, int flag)//newext正是要插入extent B+数的ext4_extent
{
	struct ext4_extent_header *eh;
    ...
    //ext4 extent B+树深度
	depth = ext_depth(inode);
    //ext4 extent B+树叶子节点中起始逻辑块地址最接近map->m_lblk这个起始逻辑块地址的ext4_extent
	ex = path[depth].p_ext;
	eh = path[depth].p_hdr;
...
prepend:
        ...
        //测试newext和它后边的ex这两个ext4_extent的逻辑块和物理块地址是否紧挨着，是则合并二者逻辑块地址并返回1，此时ex += 1了，
        //不再是path[depth].p_ext那个ext4_extent结构
        /*参与合并的两个ext4_extent必须是initialized状态，否则无法合并*/
		if (ext4_can_extents_be_merged(inode, newext, ex)) {
			...
			if (ext4_ext_is_uninitialized(ex))
				uninitialized = 1;
            //把ex的逻辑块地址范围合并到newext，还是以ex为母体呀
			ex->ee_block = newext->ee_block;
            //更新ex的起始物理块地址为newext的物理块地址
			ext4_ext_store_pblock(ex, ext4_ext_pblock(newext));
            //ex->ee_len增加newext的逻辑块个数
			ex->ee_len = cpu_to_le16(ext4_ext_get_actual_len(ex)
					+ ext4_ext_get_actual_len(newext));
			if (uninitialized)
				ext4_ext_mark_uninitialized(ex);
	...
	depth = ext_depth(inode);
	eh = path[depth].p_hdr;
    //eh->eh_max是ext4_extent B+树叶子节点最大ext4_extent个数，这是测试path[depth].p_hdr所在叶子节点的ext4_extent结构是否爆满，
    //没有超过才会跳到has_space分支
	if (le16_to_cpu(eh->eh_entries) < le16_to_cpu(eh->eh_max))
		goto has_space;

    /*到这里说明ext4_extent B+叶子节点空间不够了*/
    ...
    //如果要插入的newext起始逻辑块地址大于ext4 extent B+树叶子节点最后一个ext4_extent结构的，说明当前的叶子节点逻辑块地址范围太小了
	if (le32_to_cpu(newext->ee_block) > le32_to_cpu(fex->ee_block))
	/*回到eh这个叶子节点上层的索引节点，找到path[depth].p_idx指向的ext4_extent_idx，这个索引节点结构ext4_extent_idx的起始逻辑块地址
	最接近传入的逻辑块地址map->m_lblk，接着找到紧挨着这个ext4_extent_idx结构后边的ext4_extent_idx，这个ext4_extent_idx的起始
	逻辑块地址就可能大于newext->ee_block，即本次要插入ext4 extent B+树的ext4_extent(即newext)的起始逻辑块地址。这样的话newex就插入
	新找到的索引节点ext4_extent_idx的下边的叶子节点，最后返回这个新的ext4_extent_idx的起始逻辑块地址。
	如果找不到这个索引节点ext4_extent_idx，返回EXT_MAX_BLOCKS*/
	...
	if (next != EXT_MAX_BLOCKS) {//成立说明找到了合适的ext4_extent_idx
        ...
        //叶子节点的ext4_extent树没有超过eh->eh_max成立，即叶子节点ext4_extent结构没有爆满
		if (le16_to_cpu(eh->eh_entries) < le16_to_cpu(eh->eh_max)) {
...
块范围太小，newext无法插入。2:ext4 extent B+树叶子结点保存的ext4_extent结构爆满，没有空间了，需要增大树层数

还有一种情况可能要考虑，ext4 extent B+树是空的!在刚读写文件时，B+树层数是0或者1，此时叶子结点ext4_extent结构很容易就爆满了，
...
/*执行到这里，说明ext4 extent B+树中与newext->ee_block有关的叶子节点ext4_extent结构爆满了，即没有空闲entry了。

这里说明一点，判断叶子节点ext4_extent结构爆满的有两处代码，上边的if (le16_to_cpu(eh->eh_entries) < le16_to_cpu(eh->eh_max))和
if (le16_to_cpu(eh->eh_entries) < le16_to_cpu(eh->eh_max))。前者是判断path[depth].p_ext所在叶子节点ext4_extent结构是否爆满，
后者是在path[depth].p_ext所在叶子节点ext4_extent结构爆满情况下，执行next = ext4_ext_next_leaf_block(path)去上层索引节点找起始
逻辑块地址更大的ext4_extent_idx， 结果这个ext4_extent_idx下边的叶子节点的ext4_extent结构竟然也爆满。

继续上边中断的地方:首先尝试搜索叶子节点上的每一层
索引节点有没有空闲entry的，有的话记录这一层索引节点的深度是at。接着执行ext4_ext_split():从ext4 extent B+树at那一层索引节点到叶子
节点，针对每一层都创建新的索引节点，也创建叶子节点。还会尝试把索引节点path[at~depth-1].p_hdr指向的ext4_extent_idx结构的后边的
ext4_extent_idx结构和path[depth].p_ext指向的ext4_extent结构后边的ext4_extent结构，移动到新创建的叶子节点和索引节点。这样就可能保证
ext4 extent B+树中，与newext->ee_block有关的叶子节点有空闲entry，能存放newext。如果ext4 extent B+树索引节点的ext4_extent_idx结构
...
/*到这里，最新的path[depth].p_ext所在叶子节点肯定有空闲entry，即空闲位置可以存放newext这个ext4_extent结构，则直接把newext插入到叶子节点
某个ext4_extent位置处*/
has_space:
    //nearex指向起始逻辑块地址最接近 newext->ee_block这个起始逻辑块地址的ext4_extent，newext是本次要ext4 extent b+树的ext4_extent
	...
	if (!nearex) {//path[depth].p_ext所在叶子节点还没有使用过一个ext4_extent结构
	    ...
	    //newext的起始逻辑块地址大于nearex的起始逻辑块地址
        ...
        //这是计算nearex这个ext4_extent结构到叶子节点最后一个ext4_extent结构(有效的)之间的ext4_extent结构个数。注意"有效的"3个字，
		...
		if (len > 0) {
            ...
            //nearex这个ext4_extent结构的空间，下边正是把newext插入到这里，这样终于把newex插入ext4_extent B+树了
			memmove(nearex + 1, nearex,
				len * sizeof(struct ext4_extent));
		}
	}

    /*下边是把newext的起始逻辑块地址、起始物理块起始地址、逻辑块地址映射的物理块个数等信息赋值给nearex，相当于把newext添加到
    叶子节点原来nearex的位置。然后叶子节点ext4_extent个数加1。path[depth].p_ext指向newext*/
...
merge:
	...
	if (!(flag & EXT4_GET_BLOCKS_PRE_IO))
      //尝试把ex后的ext4_extent结构的逻辑块和物理块地址合并到ex。并且，如果ext4_extent B+树深度是1，并且叶子结点有很少的ext4_extent结构，
			     ...
			     int flags)
{
    ...
    //ext4 extent B+树叶子节点中起始逻辑块地址最接近map->m_lblk这个起始逻辑块地址的ext4_extent
	ex = path[depth].p_ext;
    //ex这个ext4_extent结构代表的起始逻辑块地址
	ee_block = le32_to_cpu(ex->ee_block);
    //ex这个ext4_extent结构代表的映射的物理块个数
	ee_len = ext4_ext_get_actual_len(ex);
    //ee_block是ex起始逻辑块地址，split是分割点的逻辑块地址，split大于ee_block，二者都在ex这个
    ...
    */
    ...
    /*重点，标记ex->ee_len为映射的block数，这样ex就是被标记初始化状态了，因为ex->ee_len只要不是没被标记EXT_INIT_MAX_LEN，
    就是初始化状态，但是一旦下边执行ext4_ext_mark_uninitialized(ex)，ex又成未初始化状态了*/
	...
	ex2 = &newex;//ex2就是ex分割后的后半段的逻辑块范围对应的ext4_extent结构
	ex2->ee_block = cpu_to_le32(split);//ex2的逻辑块起始地址，分割点的逻辑块地址
	ex2->ee_len   = cpu_to_le16(ee_len - (split - ee_block));//ex2逻辑块个数
	ext4_ext_store_pblock(ex2, newblock);//ex2的起始物理块地址
	if (split_flag & EXT4_EXT_MARK_UNINIT2)
		ext4_ext_mark_uninitialized(ex2);/*标记ex2未初始化状态*/

    //把分割的后半段ext4_extent结构即ex2添加到ext4 extent B+树，重点函数，作用源码里详细注释了
...
1:如果 map->m_lblk +map->m_len 小于ee_block + ee_len，即map的结束逻辑块地址小于ex的结束逻辑块地址。则把ex的逻辑块范围分割成3段
 ...
 因为split_flag1 |= EXT4_EXT_MARK_UNINIT1|EXT4_EXT_MARK_UNINIT2，则还要标记这两个ext4_extent结构"都是未初始化状态"。然后把后半段
 ...
 里边的if (uninitialized)成立，if (uninitialized)里边的split_flag1 |= EXT4_EXT_MARK_UNINIT1，可能不会加上EXT4_EXT_MARK_UNINIT2标记。
 ...
 ex，但是逻辑块数减少了(ee_block+ee_len)-map->m_lblk个。因为此时split_flag1有EXT4_EXT_MARK_UNINIT1标记，可能没有
 EXT4_EXT_MARK_UNINIT2标记，则再对ex加上"未初始化状态"，后半段的ext4_extent可能会被去掉"未初始化状态"，因为split_flag1可能没有
 EXT4_EXT_MARK_UNINIT2标记。接着，把后半段的ext4_extent结构添加到ext4 extent B+树。

 这里有个特例，就是 if (map->m_lblk >= ee_block)里的map->m_lblk == ee_block，即map的要映射的起始逻辑块地址等于ex的起始逻辑块地址，
 则执行ext4_split_extent_at()函数时，不会再分割ex，里边if (split == ee_block)成立，会执行ext4_ext_mark_initialized(ex)标记ex是
 "初始化状态"，ex终于转正了。
...
2:如果 map->m_lblk +map->m_len 大于等于ee_block + ee_len，即map的结束逻辑块地址大于ex的结束逻辑块地址。则把ex的逻辑块范围分割成2段
 ...
 (ee_block + ee_len) - map->m_lblk个逻辑块，即allocated =(ee_block + ee_len) - map->m_lblk。这个分割过程就是1.2的流程，看1.2节就行。
*/
static int ext4_split_extent(handle_t *handle,
			      struct inode *inode,
			      struct ext4_ext_path *path,
			      ...
			      int flags)
{
    ...
    //和~map->m_lblk~(map->m_lblk+map->m_len)两段，后边的map->m_lblk~(map->m_lblk+map->m_len)这map->m_len个逻辑块对应的ext4_extent
    //最终完成了map的逻辑块与物理块的映射。
	if (map->m_lblk + map->m_len < ee_block + ee_len) {
	    ...
	    //ee_block~map->m_lblk 和 map->m_lblk~(ee_block + ee_len)。map->m_lblk~(ee_block + ee_len)是map本次映射的逻辑块，没有达到map->len个
	...
	if (map->m_lblk >= ee_block) {
        ...
        重新在ext4 extent B+树查找逻辑块地址范围接近map->m_lblk的索引节点和叶子结点，到这里的ext4_split_extent_at()，path[depth].p_ext
        指向的ext4_extent结构逻辑块地址可能变了。*/
		err = ext4_split_extent_at(handle, inode, path,
...
/*如果本次要映射的物理块数(或者逻辑块数)map->len小于ex已经映射的逻辑块数ee_len，则尝试把ex的map->len的逻辑块合并到它前边或者后边的
ext4_extent结构(即abut_ex)。合并条件苛刻，需要二者逻辑块地址和物理块地址紧挨着等等。如果合并成功直接从ext4_ext_convert_to_initialized()
函数返回。否则执行ext4_split_extent()把ex的逻辑块地址进程分割成2段或者3段，分割出的以map->m_lblk为起始地址且共allocated个逻辑块的逻辑块
范围就是我们需要的，这allocated个逻辑块可以保证映射了物理块。但allocated<=map->len，即并不能保证map要求映射的map->len个逻辑块全映射完成。
注意，ext4_split_extent()对ex分割后，还剩下其他1~2段逻辑块范围，则要把它们对应的ext4_extent结构插入的ext4_extent B+树。
*/
					   ...
					   int flags)
{
    ...
    //ext4 extent B+树深度
	depth = ext_depth(inode);
    //指向ext4 extent B+树叶子节点头结构ext4_extent_header
	eh = path[depth].p_hdr;
    //ext4 extent B+树叶子节点，指向起始逻辑块地址最接近map->m_lblk这个起始逻辑块地址的ext4_extent
	ex = path[depth].p_ext;
    //ex这个ext4_extent结构的起始逻辑块地址
	ee_block = le32_to_cpu(ex->ee_block);
    //ex这个ext4_extent结构映射的物理块个数
	ee_len = ext4_ext_get_actual_len(ex);
	zero_ex.ee_len = 0;
    ...
    /*下边这两个大的if判断，在要求映射的物理块数map_len要小于ex已经映射的物理块数ee_len的情况下，如果ex前边或者后边的ext4_extent
    结构abut_ex，与ex的逻辑块地址紧挨着，则尝试把ex的map_len的逻辑块合并到abut_ex,分两种情况。1:ex的起始逻辑块地址是它前边的
    ext4_extent即abut_ex的结束逻辑块地址;2 ex的结束逻辑块地址是它后边的ext4_extent即abut_ex的起始逻辑块地址。这两种情况，都是
    把ex靠前或者靠后的map_len个逻辑块合并到abut_ex，ex的逻辑块个数只剩下ee_len-map_len。合并后，ex还是设置成未初始化状态,
    ，abut_ex保持初始化状态。allocated是abut_ex增加的逻辑块个数map_len，如果没有发生合并则allocated保持0*/
	...
	if ((map->m_lblk == ee_block) &&
		...
		abut_ex = ex - 1;
		//上一个struct ext4_extent结构即abut_ex代表的起始逻辑块地址
		prev_lblk = le32_to_cpu(abut_ex->ee_block);
        //上一个struct ext4_extent结构即abut_ex映射的物理块个数
		prev_len = ext4_ext_get_actual_len(abut_ex);
        //上一个struct ext4_extent结构即abut_ex代表的起始物理块地址
		...
		/*ex前边的ext4_extent是abut_ex。abut_ex已经初始化过，并且abut_ex和ex紧挨着，并且
        map要求映射的物理块数map_len要小于ex已经映射的物理块数ee_len。此时，abut_ex吞并了ex的
			...
			(prev_len < (EXT_INIT_MAX_LEN - map_len))) /*C4*/
		{
            ...
            //下边是重新划分ex这个ext4_extent结构的逻辑块地址范围，把之前ee_block~ee_block+map_len
            //划分给abut_ex这个ext4_extent，ex新的逻辑块地址范围是(ee_block + map_len)~
            //(ee_block + ee_len)。ex映射的逻辑块(物理块)个数减少了map_len个，abut_ex的增加了map_len个
			...
			abut_ex->ee_len = cpu_to_le16(prev_len + map_len);//abut_ex映射的物理块个数增加map_len个

			/* Result: number of initialized blocks past m_lblk */
            //allocated是abut_ex增多的逻辑块个数
			allocated = map_len;
		}
	} 
    //map要映射的结束逻辑块地址map->m_lblk+map_len等于ex的结束逻辑块地址ee_block + ee_len
		...
		abut_ex = ex + 1;
        //下一个struct ext4_extent结构即abut_ex代表的起始逻辑块地址
		next_lblk = le32_to_cpu(abut_ex->ee_block);
        //下一个struct ext4_extent结构即abut_ex映射的物理块个数
		next_len = ext4_ext_get_actual_len(abut_ex);
        //下一个struct ext4_extent结构即abut_ex代表的起始物理块地址
    ...
    /*对上边ex和abut_ex的合并再做个总结:
    1:如果本次要映射的逻辑块范围map->m_lblk ~ (map->m_lblk+map->m_len)在ex(即path[depth].p_ext指向的ext4_extent)
    的逻辑块范围内，即在ee_block~(ee_block + ee_len)范围内。
    2:map->m_lblk==ee_block~或者map->m_lblk+map->m_len==ee_block + ee_len，即map的逻辑块范围在ex的逻辑块范围最开头或者最结尾。
    3:ex与它前后或者后边ext4_extent(即abut_ex)的逻辑块地址紧挨着。

    以上条件都满足时，则把ex逻辑块范围内的本次要映射的逻辑块map->m_len个逻辑块合并到abut_ex。因为本次要映射的逻辑块的起始逻辑块
    地址map->m_lblk或者结束逻辑块地址map->m_lblk+map->m_len与abut_ex的结束逻辑块地址或起始逻辑块地址紧挨着。如此，相当于把本次
    要映射的逻辑块map->m_lblk ~ (map->m_lblk+map->m_len)从ex剥离掉，全合并到abut_ex，合并map->m_len个逻辑块。为什么要这样操作?
    因为abut_ex是标记已初始化状态的，ex是未初始化状态，而映射的逻辑块范围map->m_lblk ~ (map->m_lblk+map->m_len)又在ex的最开头
    或者最结尾，把它合并到abut_ex就完工了，后续就可以给ext4 extent tree使用了。最后，ex依然被标记未初始化状态。
    如果map合并到abut_ex map->m_len个逻辑块，则allocated=map->m_len，直接goto out退出该函数。
    ...
    if (allocated) {//allocated非0说明abut_ex逻辑块范围吞并了ex map_len个逻辑块
        ...
        /*ext4 extent叶子节点变为abut_ex，原来的ex废弃了，隐藏知识点*/
...
//重点在里边的ext4_ext_convert_to_initialized()函数
static int
			...
			struct ext4_map_blocks *map, int flags)
{
	...
	/*在ext4 extent B+树每一层索引节点(包含根节点)中找到起始逻辑块地址最接近传入的起始逻辑块地址map->m_lblk的ext4_extent_idx
	结构保存到path[ppos]->p_idx.然后找到最后一层的叶子节点中最接近传入的起始逻辑块地址map->m_lblk的ext4_extent结构，
	保存到path[ppos]->p_ext。这个ext4_extent才包含了逻辑块地址和物理块地址的映射关系。*/
	...
	if (ex) {
        ...
        ////如果map->m_lblk在ex的逻辑块地址范围内，奇怪，为什么只是map->m_lblk，没有map->m_lblk+m_len呢?map->m_lblk~map->m_lblk+
		...
		if (in_range(map->m_lblk, ee_block, ee_len)) {
            ...
            //allocated : map->m_lblk到(ee_block+ee_len)这个范围的block个数
			...
			ret = ext4_ext_handle_uninitialized_extents(//高版本内核 函数名称改为 ext4_ext_handle_unwritten_extents()
...
got_allocated_blocks:
    ...
    //设置本次映射的map->m_len个物理块的起始物理块号(newblock)到newex，newex是针对本次映射分配的ext4_extent结构
	ext4_ext_store_pblock(&newex, newblock + offset);//offset是0
	//设置newex映射的物理块个数，与执行ext4_ext_mark_initialized()标记ex已初始化一个效果
