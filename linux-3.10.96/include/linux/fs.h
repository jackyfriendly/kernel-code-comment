...
/*
块设备对应的block_device 结构，在 bdget->iget5_locked分配
主分区mmclk0的bdev生成流程 add_disk->register_disk->bdget_disk->bdget
其他分区mmcblk0p4 block_device生成流程 vfs_kern_mount->mount_fs->cramfs_mount->mount_bdev
->blkdev_get_by_path->lookup_bdev->bd_acquite->bdget
一个块设备有多个bdev，主分区mmcblk0一个，其他分区各一个
*/
struct block_device {
    //包含块设备的主次设备号
	dev_t			bd_dev;  /* not a kdev_t - it's a search key */
	int			bd_openers;//open计数
	//块设备主分区mmcblk0和其他分区mmcblk0p4各有一个inode，不是同一个，bd_inode就指向块设备
    //的inode,bdget函数赋值
	struct inode *		bd_inode;	/* will die */
	struct super_block *	bd_super;//bdev文件系统的超级快，mount_bdev函数赋值
...
/*
块设备的inode，/dev/mmcblk0对应的代表块设备的inode，在bdget->iget5_locked分配
创建inode时用到超级快的函数alloc_inode()中 sb->s_op->alloc_inode
*/
struct inode {
    //文件属性，代表文件或者目录或者链接符号
	umode_t			i_mode;
    ...
    //cgroup是cgroup_dir_inode_operations和cgroup_file_inode_operations，在cgroup_create_file()中设置
	const struct inode_operations	*i_op;
	struct super_block	*i_sb;//超级快
	//块设备的i_mapping是在blkdev_open->bd_acquire 中赋值
    ...
    /*
      就是主次设备号集合，块设备字符设备都有，major+minor ,/dev/mmcblk0p5的inode->i_rdev是在
      shmem_get_inode->init_special_inode 赋值，块设备mmcblk0p5 inode->i_rdev在bdget函数赋值
      这个i_rdev是连接 /dev/mmcblk0p5 inode 和块设备mmcblk0p5 inode的桥梁，前者属于tmpfs文件
      系统，后者是bdev文件系统，与文件数据存储的ext4文件系统又不是一回事，二者都是块设备的
      主次设备号，相等的
      */
	dev_t			i_rdev;
	...
	union {
        //dentry结构通过d_alias挂入inode的i_dentry链表
    ...
    /*
     取值有ext4_file_operations、ext4_dir_operations、def_chr_fops字符设备的def_blk_fops、
     def_blk_fops块设备的。感觉inode的i_fop就是底层文件系统open、read、write的函数集合，
     块设备字符设备在shmem_get_inode->init_special_inode赋值。
       ext4文件系统的在ext4_iget赋值�
      */
	const struct file_operations	*i_fop;	/* former ->i_op->default_file_ops *///cgroup是simple_dir_operations，cgroup_create_file()中设置
	struct file_lock	*i_flock;
    //mmclbk0p5块设备的inode的struct address_spacede i_data的a_ops是def_blk_aops，bdget函数赋值
	...
	union {
        ...
        ///dev/mmcblk0p5的inode的i_bdev是在blkdev_open->bd_acuire中赋值，块设备mmcblk0p5的
        //inode的i_bdev在blkdev_open->bd_acquire->bdget赋值
...
struct file {
    ...
    //保存文件所处文件系统的根目录dentry，vfsmount，文件的dentry
	struct path		f_path;
#define f_dentry	f_path.dentry
	struct inode		*f_inode;	/* cached value */
    /*字符设备的是在chrdev_open函数里filp->f_op=fops_get(p->ops).ext4 其他的在do_dentry_open
      filp->f_op=fops_get(inode->ops)获得inode的file_operation，ext4_file_operations所以*/
...
struct super_block {
	...
	struct dentry		*s_root;//顶层目录的dentry结构
	...
	const struct dentry_operations *s_d_op; /* default d_op for dentries *///cgroup是cgroup_dops()
...
//cgroup系统的是cgroup_file_operations
struct file_operations {
	struct module *owner;
	loff_t (*llseek) (struct file *, loff_t, int);
	ssize_t (*read) (struct file *, char __user *, size_t, loff_t *);
    //cgroup写文件时调用的是cgroup_file_write()
...
//cgroup子系统的有cgroup_dir_inode_operations和cgroup_file_inode_operations
